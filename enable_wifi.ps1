# enable_wifi.ps1
# 專為 Windows PowerShell 5.1 設計的非同步 WinRT Wi-Fi 啟動腳本 (免系統管理員權限)
...
try {
    ...
    if ($wifi) {
        ...
        $status = Await-WinRt ($wifi.SetStateAsync(1)) ([Windows.Devices.Radios.RadioAccessStatus])
        Write-Host "Wi-Fi radio successfully enabled."
    } else {
        Write-Warning "No Wi-Fi adapter found."
    }
} catch {
    Write-Warning "Failed to enable Wi-Fi radio: $_"
}
